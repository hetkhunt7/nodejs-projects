...
const fs = require('fs');
const SOURCE_FILE = 'highscores.json';

function loadScore() {
    if(!fs.existsSync(SOURCE_FILE)) return {Easy : null, Medium : null, Hard : null};
    const data = fs.readFileSync(SOURCE_FILE, 'utf-8').trim();
    if(!data) return {Easy : null, Medium : null, Hard : null};
    return JSON.parse(data);
}

function saveScore(scores) {
    fs.writeFileSync(SOURCE_FILE, JSON.stringify(scores, null, 2));
}

function UpdateHighScore(difficulty, attempt) {
    const scores = loadScore();
    const current = scores[difficulty];

    if(current == null || current > attempt) {
        scores[difficulty] = attempt;
        saveScore(scores);
        console.log(`New high score for ${difficulty}: ${attempt} attempt(s)!\n`);
    } else {
        console.log(`High score for ${difficulty}: ${current} attempt(s).\n`);
    }
}
    ...
    while (true) {
        ...
        while(attempt < chance) {
            ...
            if (guess === secret) {
                ...
                won = true;
                UpdateHighScore(difficultyName, attempt);
                break;
            }
        ...
        const again = await ask('Play again? (yes/no): ');
